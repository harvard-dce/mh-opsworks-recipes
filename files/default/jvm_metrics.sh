...
echo $data | while read survivor0 survivor1 eden oldgen metaspace compclass younggc younggctime fullgc fullgctime totalgctime
do
    ...
    aws cloudwatch put-metric-data --metric-name="MetaspaceUsage" --value="$metaspace" --namespace="$namespace" --dimensions="InstanceId=$instance_id" --unit Percent --region="$region"
    aws cloudwatch put-metric-data --metric-name="CompressedClassUsage" --value="$compclass" --namespace="$namespace" --dimensions="InstanceId=$instance_id" --unit Percent --region="$region"
