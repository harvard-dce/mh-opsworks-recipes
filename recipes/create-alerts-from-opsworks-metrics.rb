...
ruby_block "add alarms" do
  block do
    ...
    if ::File.exists?('/etc/mdadm/mdadm.conf')
      ...
      command = %Q(aws cloudwatch put-metric-alarm --region "#{region}" --alarm-name "#{alarm_name_prefix}_raid_array_sync" --alarm-description "Software RAID arrays out of sync on #{alarm_name_prefix}" --metric-name RAIDArrayInSync --namespace AWS/OpsworksCustom --statistic Minimum --period 120 --threshold 1 --comparison-operator LessThanThreshold --dimensions Name=InstanceId,Value=#{aws_instance_id} --evaluation-periods 1 --alarm-actions "#{topic_arn}" --ok-actions "#{topic_arn}")
      Chef::Log.info command
      %x(#{command})
    end
    ...
    if admin_node?
      ...
      command = %Q(aws cloudwatch put-metric-alarm --region "#{region}" --alarm-name "#{alarm_name_prefix}_mysql_availablity" --alarm-description "MySQL is unavailable #{alarm_name_prefix}" --metric-name MySQLServerAvailable --namespace AWS/OpsworksCustom --statistic Minimum --period 120 --threshold 1 --comparison-operator LessThanThreshold --dimensions Name=InstanceId,Value=#{aws_instance_id} --evaluation-periods 1 --alarm-actions "#{topic_arn}" --ok-actions "#{topic_arn}")
      Chef::Log.info command
      %x(#{command})
    end
  end
