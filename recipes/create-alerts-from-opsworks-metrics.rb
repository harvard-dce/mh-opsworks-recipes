...
ruby_block "add alarms" do
  block do
    ...
    total_ram = execute_command(%Q(grep MemTotal /proc/meminfo | sed -r 's/[^0-9]//g')).chomp.to_i
    local_file_systems = execute_command(%Q(mount | grep -E 'type ext|type xfs' | cut -f3 -d' ')).chomp.split(/\n/)
    nfs_file_systems = execute_command(%Q(mount | grep -E 'type nfs' | cut -f3 -d' ')).chomp.split(/\n/)
