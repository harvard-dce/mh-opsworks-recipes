...
if dev_or_testing_cluster?
  ...
  if do_it && admin_node?
    ...
    execute 'modify database to reflect local cluster hostnames' do
      user "root"
      command %Q|/usr/local/bin/modify-database-after-loading-seed-data.sh -x -p "#{shared_storage_root}" --engage_ip "#{engage_ip}" --engage_host "#{engage_host}" --admin_ip "#{admin_ip}" --admin_host "#{admin_host}" --cloudfront_domain "#{cloudfront_domain}" --wowza_edge_url "#{wowza_edge_url}"|
      timeout 600
    end
  end
end
